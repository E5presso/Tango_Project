	...
	class Program
	{
		...
		private static readonly Thread thread = new Thread(new ThreadStart(() =>
		{
			while (true)
			{
				...
				float x1 = Convert.ToSingle(string.Format("{0:F2}", (7.0f * (float)random.NextDouble()) - 3.5f));
				float x2 = Convert.ToSingle(string.Format("{0:F2}", (7.0f * (float)random.NextDouble()) - 3.5f));
				int i1 = (int)(x1 * 100);
				int i2 = (int)(x2 * 100);
				buffer.Write(0x00);
				buffer.Write(0x00);
